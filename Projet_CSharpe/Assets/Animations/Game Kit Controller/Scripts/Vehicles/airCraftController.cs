...
public class airCraftController : MonoBehaviour {
	...
	public float maxEnginePower = 40;        // The maximum output of the engine.
	public float liftAmount = 0.002f;               // The amount of lift generated by the aeroplane moving forwards.
	public float stopLiftAtSpeed = 300;         // The speed at which lift is no longer applied.
	public float pitchForce = 0.75f;            // The strength of effect for pitch input.
	public float yawForce = 0.75f;            // The strength of effect for yaw input.
	public float rollForce = 0.75f;             // The strength of effect for roll input.
	public float bankedTurnForce = 0.5f;     // The amount of turn from doing a banked turn.
	public float aeroDynamicForce = 0.02f;   // How much aerodynamics affect the speed of the aeroplane.
	public float bankedTurnPitch = 0.5f;        // How much the aeroplane automatically pitches when in a banked turn.
	public float bankedRollForce = 0.1f;        // How much the aeroplane tries to level when not rolling.
	public float bankedPitchForce = 0.1f;       // How much the aeroplane tries to level when not pitching.
	public float brakesForce = 20;        // How much the air brakes effect the drag.
	public float throttleChangeSpeed = 0.3f;  // The speed with which the throttle changes.
	...
	void Start() {
		...
		foreach (Component c in components) {
			...
			newExhaust.startSpeed = (c as ParticleSystem).startSpeed;
		...
		vehicleGravityManager = GetComponent<vehicleGravityControl> ();

		playAnimation (true, vehicleParts.wings, settings.wingsAnimation);
		interfaceInfo = GetComponent<shipInterfaceInfo> ();
	...
	void Update() {
		if (driving) {
			...
			//boost input
			if (actionManager.getActionInput ("Enable Turbo")) {
				usingBoost = true;
			...
			braking = actionManager.getActionInput ("Brake");
			if (actionManager.getActionInput ("Rotate To Left")) {
				rollDirection = 1;
			} else if (actionManager.getActionInput ("Rotate To Right")) {
				rollDirection = -1;
			} else {
				rollDirection = 0;
			}
			if (actionManager.getActionInput ("Land Or Take Off")) {
			...
			if (usingBoost) {
				...
				//else, disable the boost
					...
					if (!vehicleGravityManager.powerActive) {
						vCamera.usingBoost (false,"Boost");
					}
					usingBoosting ();
					boostInput = 1;
	...
	IEnumerator placeOnGroundOrOnAir(bool state){
		if (state) {
			...
			if (canLand) {
				...
				playAnimation (true, vehicleParts.wings, settings.wingsAnimation);
				StartCoroutine( vehicleGravityManager.rotateVehicleToLandSurface (hit.normal,hit.point));
				StartCoroutine (enableOrDisableTrailsState (false));
			}
		}
		yield return null;
	...
	public void playAnimation (bool forward, GameObject obj, string name){
		if (forward) {
			obj.GetComponent<Animation>() [name].speed = 1; 
			obj.GetComponent<Animation> ().Play (name);
		} else {
			obj.GetComponent<Animation>() [name].speed = -1; 
			obj.GetComponent<Animation> () [name].time = obj.GetComponent<Animation> () [name].length;
			obj.GetComponent<Animation> ().Play (name);
		}
	}
